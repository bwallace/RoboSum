...
#def create_position_ids_from_input_ids(input_ids, padding_idx,nxt_token):
def create_position_ids_from_input_ids(input_ids, padding_idx):
    ...
    fill=torch.zeros_like(mask)
    pos, dim = mask.shape
    for i in range(pos):
        c=1
        for j in range(dim):
            if input_ids[i,j]==nxt_token:
                c=1
                fill[i,j]=c
            else:
                fill[i,j]=c
    ...
    incremental_indices = fill.type_as(mask) * mask"""
    mask = input_ids.ne(padding_idx).int()
    incremental_indices = torch.cumsum(mask, dim=1).type_as(mask) * mask
    return incremental_indices.long() + padding_idx

def create_position_ids_from_input_ids_bart(input_ids, padding_idx,nxt_token):
    """ Replace non-padding symbols with their position numbers. Position numbers begin at
    padding_idx+1. Padding symbols are ignored. This is modified from fairseq's
    `utils.make_positions`.

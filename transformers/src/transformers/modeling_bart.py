...
from .modeling_utils import PreTrainedModel, create_position_ids_from_input_ids,create_position_ids_from_input_ids_bart
...
class BartEncoder(nn.Module):
    ...
    def __init__(self, config: BartConfig, embed_tokens):
        ...
        if config.static_position_embeddings:
            ...
            self.embed_positions = LearnedPositionalEmbeddingNew(
                config.max_position_embeddings, embed_dim, self.padding_idx,0
            )
...
class BartDecoder(nn.Module):
    ...
    def __init__(self, config: BartConfig, embed_tokens: nn.Embedding):
        ...
        if config.static_position_embeddings:
            ...
            self.embed_positions = LearnedPositionalEmbeddingNew(
                config.max_position_embeddings, config.d_model, self.padding_idx,0
            )
            """self.embed_positions = LearnedPositionalEmbedding(
...
class LearnedPositionalEmbeddingNew(nn.Embedding):
    ...
    def __init__(
        self, num_embeddings: int, embedding_dim: int, padding_idx: int,nxt_token
    ):
        # if padding_idx is specified then offset the embedding ids by
        # this index and adjust num_embeddings appropriately
        self.nxt_token=nxt_token
        assert padding_idx is not None
        num_embeddings += padding_idx + 1  # WHY?
        super().__init__(num_embeddings, embedding_dim, padding_idx=padding_idx)
    ...
    def forward(self, input, use_cache=False):
        ...
        if use_cache:  # the position is our current step in the decoded sequence
            ...
            positions = create_position_ids_from_input_ids_bart(input, self.padding_idx,self.nxt_token)
            #positions = create_position_ids_from_input_ids(input, self.padding_idx)
        return super().forward(positions)
...
class BartForConditionalGeneration(PretrainedBartModel):
    ...
    Examples::
        ...
        #print("output") # outputs 0: decoder output, output1 is encoder output
        #print(outputs[0].shape) # shape of bz,sq ,hidden
        #print(outputs[1].shape) # spape bz, hidden, hidden
        lm_logits = F.linear(outputs[0], self.model.shared.weight, bias=self.final_logits_bias)
        #print("lm logits")
        #print(lm_logits.shape) # spape bz, hidden, vocab_length

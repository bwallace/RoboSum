...
class SummarizationTrainer(BaseTransformer):
    ...
    def _step(self, batch):
        ...
        y_ids = y[:, :].contiguous()
        lm_labels = torch.ones_like(y_ids)
        lm_labels[:,:-1]=y[:,1:].clone()
        #lm_labels = y[:, 1:].clone()
    ...
    def test_step(self, batch, batch_idx):
        ...
        generated_ids = self.model.generate(
            ...
            num_beams=4,
            max_length=110, # change to 110
            repetition_penalty=2.5,
            length_penalty=5.0,
            early_stopping=True,
            do_sample=True # remove
    ...
    def get_dataloader(self, type_path: str, batch_size: int, shuffle: bool = False) -> DataLoader:
        ...
        dataloader = DataLoader(dataset, batch_size=batch_size, collate_fn=dataset.collate_fn, shuffle=shuffle,drop_last=True,pin_memory=True,num_workers=5)
        return dataloader
...
def main(args):
    ...
    if args.do_predict:
        ...
        checkpoints = os.path.join(args.output_dir,(args.output_dir.split('/')[-1]+ ".ckpt"))
        print(checkpoints)
